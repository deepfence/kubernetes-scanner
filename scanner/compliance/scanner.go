...
func (c *ComplianceScanner) RunComplianceScan() error {
	...
	cmd := fmt.Sprintf("su - deepfence -c 'cd %s && steampipe check --progress=false --output=none --export=%s benchmark.nsa_cisa_v1'", steampipeKubernetesCompliancePath, tempFileName)
